...
describe('CLI Integration Tests', () => {
  ...
  describe('Start Command', () => {
    ...
    it('should handle start with model option', async () => {
      const { stdout } = await execAsync('node bin/llm-mock.js start --model=gemini');
      expect(stdout).toContain('Starting LLM Mock Server with model: gemini');
      expect(stdout).toContain('Server started successfully!');
    });

    it('should handle start with port option', async () => {
      const { stdout } = await execAsync('node bin/llm-mock.js start --port=3000');
      expect(stdout).toContain('Server is running at: http://0.0.0.0:3000');
    });

    it('should handle start with multiple options', async () => {
      const { stdout } = await execAsync('node bin/llm-mock.js start --port=3000 --model=gemini --debug=true');
      expect(stdout).toContain('Server is running at: http://0.0.0.0:3000');
    });
  });
  ...
  describe('Default Behavior', () => {
    ...
    it('should default to start when only options provided', async () => {
      const { stdout } = await execAsync('node bin/llm-mock.js --port=3000');
      expect(stdout).toContain('Server is running at: http://0.0.0.0:3000');
    });
  });
  ...
  describe('Option Parsing', () => {
    it('should handle --key=value format', async () => {
      const { stdout } = await execAsync('node bin/llm-mock.js start --port=3000 --model=gemini');
      expect(stdout).toContain('Server is running at: http://0.0.0.0:3000');
    });
    ...
    it('should handle mixed option formats', async () => {
      const { stdout } = await execAsync('node bin/llm-mock.js start --port=3000 --debug true --model=gemini');
      expect(stdout).toContain('Server is running at: http://0.0.0.0:3000');
    });
  });
});
