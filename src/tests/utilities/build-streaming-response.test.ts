...
describe('build streaming response function works as expected', async () => {
    ...
    test('uses template values for id, model, and created', async () => {
        ...
        expect(firstChunk.id).to.match(/^chatcmpl-6sf37lXn5paUcuf8UaurpMIKRMsTe/);
    ...
    test('splits content into multiple chunks', async () => {
        const longContent = 'This is a very long response that should be split into multiple chunks for testing the streaming functionality.';
    ...
    test('sets correct streaming headers', () => {
        ...
        expect(mockReply.header).toHaveBeenCalledWith('Content-Type', 'text/event-stream');
        expect(mockReply.header).toHaveBeenCalledWith('Cache-Control', 'no-cache');
        expect(mockReply.header).toHaveBeenCalledWith('Connection', 'keep-alive');
        expect(mockReply.header).toHaveBeenCalledWith('Access-Control-Allow-Origin', '*');
        expect(mockReply.header).toHaveBeenCalledWith('Access-Control-Allow-Headers', 'Cache-Control');
    });
});
...
describe('streaming with delay function', async () => {
    ...
    test('handles destroyed connection gracefully', async () => {
        ...
        await expect(streamWithDelay(chunks, mockReply)).resolves.toBeUndefined();
    });
});
